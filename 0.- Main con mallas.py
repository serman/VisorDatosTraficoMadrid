...
#-------------------------------------------------------------------------------------------------------------------
  ...
  if (opcion==4): # Comparar los valores de un punto en concreto en un periodo de x días a partir de dos días dados.
    ...
    #!! Esta versión tiene un problema gordo, si no hay exáctamente el mismo número de mediciones el resultado de salida tiene poco sentido porque mezcla mediciones de horas diferentes
    #Se podría modificar para que solo muestre las mediciones en las que coincida la hora y/o que diga el % de mediciones que hay sobre las que habría de haber
